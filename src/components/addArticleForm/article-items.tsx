...
const ArticleItems = ({ content, handleContentChange, articleCreated }) => {
  ...
  const handleKey = (index: any, e: any) => {
    ...
    if (isList) {
      if (key === 'Enter') {
        ...
        if (!item.value || item.value == '<ul><li><br></li></ul>' || item.value == '<ol><li><br></li></ol>') {
          changeInputType(index, 'paragraph')
        }
      }
  ...
  const clearUrlContext = () => {
    setState((prev) => ({ ...prev, openContextLink: false, errors: {}, urlValue: '', nameLink: '' }))
  }
  ...
  const imgComposerOutline = (e: any) => {
    ...
    if (!e.target.className.includes('toggle-media') && inputsList[arrayIndex]) {
  ...
  const focus = (index: number) => {
    ...
    setTimeout(() => {
      ...
      if (index < nodes.length) {
        const nodeIndex = inputsList.findIndex((x: any) => x.index === maximumIndex)
  ...
  const checkUrl = (url: string, typeField: string = 'urlValue') => {
    ...
    if (!regex.test(new_url)) {
      setState((prevState: any) => ({ ...prevState, errors: { ...prevState.errors, [typeField]: true } }))
      return false
    }
    setState((prevState: any) => ({ ...prevState, errors: { ...prevState.errors, [typeField]: false } }))
    return new_url
  }
  ...
  const confirmLink = (e: any, index: number) => {
    ...
    inputsList[index].value = `<a href="${correct_urlValue}">${nameLink || urlValue}</a>`
    ...
    <ArticleItem>
        <div className="articles-editor__editor">
          {isTextSelected && (
            ...
            >
              <button className="articles-editor__toolbar-item" onClick={(e) => execCommand('bold', e)}>
                <BoldIcon />
              </button>
              <button className="articles-editor__toolbar-item" onClick={(e) => execCommand('italic', e)}>
                <ItalicIcon />
              </button>
            </div>
          )}
          {state?.inputsList?.map((input: any, index: number) => (
            <div className="input-item" key={input.key}>
              {!input.value && input.tooltipIcon && (
                <span className="text-tooltip toggle-tooltip" onClick={() => toggleTooltip(input.index)}>
                  {input.tooltip ? <CancelOutlinedIcon color={"primary"} /> : <AddCircleOutlineRoundedIcon color={"primary"}/>}
                </span>
              )}
              ...
              {input.tooltip && !input.value && (
                <div className="articles-editor__change-type">
                  ...
                  <span className="change-type__item subtitle" onClick={() => changeInputType(input.index, 'subtitle')}>
                    Subtitle
                  </span>
                      ...
                      <div>
                        <React.Fragment>
                          <Input
                            ...
                            onKeyDown={(e) => listenerSubmit(e, () => confirmLink(e, index))}
                          ...
                          {state.errors.urlValue && <Typography>URL is invalid</Typography>}
                        </React.Fragment>
                        <Input
                          className="editor-link__item"
                          type="text"
              ...
              {input.type === 'paragraph' && (
                <ContentEditable
                  className={
                    ...
                    (state.inputsList.length === 1 && input.index === 0 ? 'first' : '')
                  }
                  html={input.value}
                  disabled={false}
                ...
                <div>
                  {input.value && (
                    <div className="articles-editor__photo articles-editor__item toggle-media">
                      <img
                        className={'articles-editor__photo-preview toggle-media ' + (input.active ? 'active' : '')}
                        src={input.value}
              ...
              {input.type === 'youtube' && (
                <div className="articles-editor__video toggle-media">
                  {input.ytID && (
                    <div
                      className={'articles-editor__iframe-container articles-editor__item toggle-media ' + (input.active ? 'active' : '')}
                      onClick={() => toggleMediaActive(input.index)}
                    >
                      <YouTube className="articles-editor__iframe" videoId={input.ytID} />
                    </div>
                  )}
                  {input.vimeoID && (
                    <div
                      ...
                      onClick={() => toggleMediaActive(input.index)}
                    >
                      <Vimeo className="articles-editor__iframe" videoId={input.vimeoID} />
                    </div>
                  )}
